...
La règle clé est qu'il faut alterner entre les "blocs de zéros" et les "blocs de uns". Disons que nous avons $k\_0$ blocs de zéros et $k\_1$ blocs de uns au total. En raison de cette alternance, leurs comptes doivent être presque identiques.

Ils peuvent différer d'au plus un, ce qui signifie $\vert k\_0 - k\_1 \vert \le 1$. Cela peut sembler inutile à préciser, mais nous serons heureux de l'avoir fait.
...
C'est le cas où $k\_0 = k\_1$, ce qui signifie $n-i = n-j \implies i = j$.
...
Cela se produit lorsque $k\_0 = k\_1 + 1$. Puisque $k\_0 = n-i$ et $k\_1 = n-j$, nous obtenons $n-i = (n-j)+1 \implies j = i+1$.
...
L'image miroir du cas précédent. Ici, $k\_1 = k\_0 + 1$, ce qui signifie $n-j = (n-i)+1 \implies j = i-1$.
* **Les briques de construction** : $\binom{n-i}{i}$ façons pour les zéros et $\binom{n-(i-1)}{i-1} = \binom{n-i+1}{i-1}$ pour les uns.
